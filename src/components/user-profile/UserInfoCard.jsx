  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
      <div className="flex items-center justify-between">
        <h4 className="text-base font-semibold text-gray-800 subpixel-antialiased dark:text-white/90">
      ...
      <div className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2">
        {userInfo.map((info, index) => (
          <div
            key={index}
            className={`flex items-start gap-3 ${info.fullWidth ? "sm:col-span-2" : ""}`}
          >
            {info.icon && (
              <span className="mt-1 flex-shrink-0 text-gray-500">
                {info.icon}
              </span>
            )}
