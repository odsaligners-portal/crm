...
const STLUploadModal = ({ isOpen, onClose, patient, token, onSuccess }) => {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDrop: (acceptedFiles) => {
      if (acceptedFiles.length > 0) {
        ...
        // Validate file size (25MB limit for any file type)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch("/api/admin/patients/stl-upload", {
        method: "POST",
        headers: {
      ...
      <div className="h-[80vh] overflow-y-auto p-8">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Admin Upload - Planner deadline will not be affected
          </p>
        </div>
