...
const DentalExaminationForm = () => {
  ...
  useEffect(() => {
    // Load case categories when doctor is selected
    if (formData.userId) {
      const selectedDoctor = doctors.find((doc) => doc._id === formData.userId);
      if (selectedDoctor) {
        loadCaseCategories(selectedDoctor.distributerId);
      }
    } else {
      // Clear categories if no doctor selected
      setCaseCategories([]);
      setCountrySpecificCategories([]);
      setDefaultCategories([]);
    }
  }, [formData.userId, doctors]);
  ...
  const loadCaseCategories = async (distributerId = null) => {
    try {
      ...
      // If distributerId is null/undefined, don't add parameter (should not happen in admin form)
    ...
    if (
      ...
      if (name === "userId") {
        setFormData((prev) => ({
          ...prev,
          [name]: value,
          // Reset case category related fields when doctor changes
          caseCategory: "",
          selectedPrice: "",
          caseCategoryDetails: "",
        }));
      ...
      if (name === "country") {
        setFormData((prev) => ({
          ...prev,
          [name]: value,
          // Reset state and city since they depend on country
          state: "",
          city: "",
        }));
        return; // Return early to avoid setting the value again
  ...
  const handleSaveAndNext = async (nextTab) => {
    ...
    // Validate case information - removed validation for caseType, caseCategory and selectedPrice
    if (!formData.userId) {
                ...
                {caseCategories.length > 0 && (
                  <div className="rounded-2xl border border-gray-100 bg-gradient-to-br from-white to-gray-50 p-8 shadow-lg transition-all duration-300 hover:shadow-xl">
                    ...
                    <div className="space-y-6">
                      ...
                      {/* Case Category - Fetched based on selected doctor's distributor + default categories */}
                      <div className="group">
                        <label className="mb-3 block text-sm font-semibold text-gray-700 subpixel-antialiased transition-colors duration-200 group-focus-within:text-blue-600">
                          Case Category
                        </label>
