...
export default function ViewPatientDetails() {
  ...
  const fetchPatientFiles = async () => {
    try {
      ...
      const result = await fetchWithError(
        `/api/admin/patients/files?patientId=${patientId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const fetchComments = async () => {
    try {
      const patientId = searchParams.get("id");
      const result = await fetchWithError(
        `/api/admin/patients/comments?patientId=${patientId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const patientId = searchParams.get("id");
      const response = await fetch(
        `/api/admin/patients/download-pdf?id=${patientId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="group inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-green-600 to-emerald-600 px-6 py-3 font-semibold text-white shadow-lg transition-all duration-200 hover:scale-105 hover:from-green-700 hover:to-emerald-700 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:scale-100"
                >
                  {isDownloadingPDF ? (
                    <>
                      <svg
                  ...
                  <div className="mb-6 rounded-xl border border-indigo-200 bg-gradient-to-r from-indigo-50 to-purple-50 p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="text-sm font-semibold text-indigo-800 subpixel-antialiased">
                            Assignment Status
                          </h3>
                          <p className="text-xs text-indigo-600">
                            {patientData.userId?.name &&
                            patientData.plannerId?.name
                              ? "Fully assigned - Doctor and Planner assigned"
                              : patientData.userId?.name ||
                                  patientData.plannerId?.name
                                ? "Partially assigned - Only one personnel assigned"
                                : "Not assigned - No personnel assigned yet"}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
                      <div className="rounded-lg bg-white/60 p-3 backdrop-blur-sm">
                        ...
                        <div className="text-sm font-semibold text-indigo-800 subpixel-antialiased">
                      ...
                      <div className="space-y-3">
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="text-sm font-medium text-gray-900">
                              <span className="font-semibold subpixel-antialiased">
                                Name:
                              </span>{" "}
                              {patientData.userId?.name || "Not assigned"}
                            </div>
                            {patientData.userId?.name && (
                        ...
                        >
                          <div className="text-sm font-medium text-gray-900">
                            ...
                            {patientData.userId?.email || "Not available"}
                          </div>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="space-y-3">
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div className="text-sm font-medium text-gray-900">
                              ...
                              {patientData.plannerId?.name || "Not assigned"}
                            </div>
                            {patientData.plannerId?.name && (
                        ...
                        >
                          <div className="text-sm font-medium text-gray-900">
                            ...
                            {patientData.plannerId?.email || "Not available"}
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      ...
                      patientData.dentalExamination.mandibularArcShape.length >
                        0 ? (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      <h3 className="text-xl font-semibold text-amber-800 subpixel-antialiased">
                        ⭐ Production Comments
                        {specialComments.filter((c) => !c.isRead).length >
                          0 && (
                          <span className="ml-2 inline-flex h-6 w-6 items-center justify-center rounded-full bg-red-500 text-xs font-semibold text-white subpixel-antialiased">
                            {specialComments.filter((c) => !c.isRead).length}
                          </span>
                        )}
                      </h3>
                    ...
                    {specialComments.length === 0 ? (
                      <div className="rounded-xl bg-white/80 p-6 backdrop-blur-sm">
                        <div className="text-center">
                          ...
                          <h4 className="mb-2 text-lg font-semibold text-amber-800 subpixel-antialiased">
                      ...
                      <div className="space-y-4">
                          ...
                          >
                            <div className="mb-4 flex items-start justify-between">
                              <div className="flex items-center gap-3">
                                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-amber-100">
                                  <span className="text-sm font-semibold text-amber-600 subpixel-antialiased">
                            ...
                            {specialComment.doctorName && (
                              <div className="mt-3 rounded-lg bg-blue-50 p-3">
                                ...
                                <div className="text-sm font-semibold text-blue-800 subpixel-antialiased">
                                  {specialComment.doctorName}
                                </div>
                              </div>
                            )}
                        ...
                        <div className="mt-6 rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-amber-100 p-6 shadow-lg">
                          <h4 className="mb-4 flex items-center gap-2 text-lg font-semibold text-amber-800 subpixel-antialiased">
                            <svg
                              className="h-5 w-5"
                              fill="none"
                              stroke="currentColor"
                          ...
                          <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
                            <div className="rounded-xl bg-white/60 p-4 text-center backdrop-blur-sm">
                              <div className="mb-1 text-2xl font-semibold text-amber-600 subpixel-antialiased">
                            ...
                            <div className="rounded-xl bg-white/60 p-4 text-center backdrop-blur-sm">
                              <div className="mb-1 text-2xl font-semibold text-green-600 subpixel-antialiased">
                                {specialComments.filter((c) => c.isRead).length}
                              </div>
                            ...
                            <div className="rounded-xl bg-white/60 p-4 text-center backdrop-blur-sm">
                              <div className="mb-1 text-2xl font-semibold text-blue-600 subpixel-antialiased">
                                {
