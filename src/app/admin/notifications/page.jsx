  ...
  const handleNotificationClick = async (notification) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/patients/find-by-case-id?caseId=${encodeURIComponent(caseId)}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      if (response.ok) {
        ...
        if (data.patientId) {
          ...
          router.push(
            `/admin/patients/view-patient-details?id=${data.patientId}`,
          );
        } else {
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {notifications.length === 0 ? (
        ...
        <div className="space-y-4">
          {notifications.map((n) => {
            const isRead = n.recipients?.find(
              (r) => r?.user === user?.id,
            )?.read;
            return (
              <div
