...
// Scan File Component
const ScanFileComponent = ({ idx, patientData, scanLabels }) => {
  ...
  const getScanFileData = (idx) => {
    if (!patientData.scanFiles) return null;
    ...
    const key = scanFileKeys[idx];
    if (!key || !patientData.scanFiles[key]) return null;

    const files = patientData.scanFiles[key];
    return files && files.length > 0 ? files[0] : null;
  };

  const fileData = getScanFileData(idx);
    ...
    return (
      <div className="group text-center">
        <label className="mb-3 block text-sm font-semibold text-gray-700">
          {scanLabels[idx % scanLabels.length]}
        </label>
        <div className="mt-2 flex h-36 w-full items-center justify-center rounded-2xl border-2 border-dashed border-gray-300 bg-gray-50">
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-gray-500">No scan file uploaded</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="group relative mx-auto mt-2 max-w-xs">
        <div className="flex h-36 flex-col items-center justify-center rounded-2xl border-2 border-gray-200 bg-white/90 shadow-lg backdrop-blur-sm transition-all duration-300 hover:scale-105 hover:shadow-xl">
          <div className="flex h-full w-full flex-col items-center justify-center p-3">
            {/* Scan File Icon */}
            <div className="mb-3 rounded-full bg-gradient-to-br from-red-100 to-pink-100 p-3">
              <svg
            ...
            <a
              href={fileUrl}
              download={fileName}
              className="mt-2 text-xs font-medium text-red-600 underline transition-colors hover:text-red-700"
...
const FileDisplayComponent = ({ idx, patientData, imageLabels }) => {
  ...
  const getFileData = (idx) => {
    if (!patientData.dentalExaminationFiles) return null;
    ...
    if (!key || !patientData.dentalExaminationFiles[key]) return null;
...
export default function ViewPatientDetails() {
  ...
  // Get search params and Redux hooks
  ...
  const handleFileDownload = (file) => {
    if (file?.fileUrl) {
      const fileName = file.fileName || getFileNameFromUrl(file.fileUrl);
      downloadFile(file.fileUrl, fileName, file._id);
    }
  };
  ...
  }, [patientData?.caseId, activeTab, patientId, token]);

  // Fetch patient files when files tab is active
  useEffect(() => {
    if (activeTab === "files" && patientId) {
      fetchPatientFiles();
    }
          ...
          {activeTab === "files" && (
            <div className="rounded-3xl border border-white/20 bg-white/95 p-8 shadow-2xl backdrop-blur-sm">
              <div className="mb-10 text-center">
                ...
                <p className="mt-2 text-lg text-gray-600">
                  Patient images, X-rays, 3D models, and scan files
                </p>
              </div>
                  ...
                  <div className="mb-6 rounded-xl border border-blue-200 bg-blue-50 p-4">
                    <div className="flex flex-wrap items-center gap-4">
                      <div className="flex items-center gap-2">
                        <input
                          type="checkbox"
                          checked={selectedFiles.size === patientFiles.length}
                          onChange={() => {
                            if (selectedFiles.size === patientFiles.length) {
                        ...
                        <span className="text-sm font-medium text-blue-800">
                          Select All ({selectedFiles.size}/{patientFiles.length}
                          )
                        </span>
                      </div>
                        ...
                        onClick={async () => {
                          try {
                            ...
                            if (!downloadMultipleFiles) {
                              console.error(
                                "downloadMultipleFiles is not defined!",
                              );
                              return;
                            }
                      ...
                      <button
                        onClick={async () => {
                          try {
                            ...
                            console.error(
                              "Error in Scan Files download all:",
                              error,
                            );
                          }
                        }}
                              ...
                              <div className="mt-3 flex items-center gap-3">
                                <button
                                  onClick={() => {
                                    ...
                                    downloadFile(file.fileUrl, fileName, index);
                                  }}
          ...
          {activeTab === "files" && (
            <div className="space-y-6">
              <div>
                ...
                {patientData.dentalExaminationFiles &&
                patientData.dentalExaminationFiles.length > 0 ? (
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {patientData.dentalExaminationFiles.map((file, index) => (
                      <FileDisplayComponent
                        key={index}
                        file={file}
                        onDownload={() => handleFileDownload(file)}
                        isDownloading={downloadingFiles.includes(file._id)}
                      />
                    ))}
                  </div>
                ) : (
                  <div className="rounded-md bg-gray-50 p-6 text-center">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      No dental examination files have been uploaded yet.
                    </p>
                  </div>
                )}
              </div>
