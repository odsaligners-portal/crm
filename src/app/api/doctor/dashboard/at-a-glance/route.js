...
export async function GET(req) {
  ...
  try {
    const patientsOfDoctor = await Patient.find({ userId: doctorId }).select(
      "_id patientName createdAt",
    );
    const patientIds = patientsOfDoctor.map((p) => p._id);

    const recentComments = await PatientComment.find({
    ...
    const atAGlanceData = patientsOfDoctor
      .map((p) => {
